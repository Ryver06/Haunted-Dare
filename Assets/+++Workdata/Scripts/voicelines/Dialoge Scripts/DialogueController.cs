...
public class DialogueController : MonoBehaviour
{
    ...
    private void ContinueDialogue()
    {
        ...
        if (CanContinue())
        {
            ...
            {
                if (tag.StartsWith("wait:")) // if inky has #wait:
                {
                    //              parse converts string to float
                    float time = float.Parse(tag.Split(':')[1]);
                    StartCoroutine(ContinueAfterTimer(time));
                }
            }
    ...
    IEnumerator ContinueAfterTimer(float timer)
    {
